/**
 * メルカリ-Amazon販売支援ツール: フィルタリング機能
 * このスクリプトはNGワードによるフィルタリングを担当します
 */
...
const Filter = (() => {
    ...
    const _config = {
      ngWords: [], // NGワードリスト
      ngSellers: [], // NG出品者リスト
      hideNgItems: true, // NGアイテムを非表示にするかどうか
      initialized: false
    };
    ...
    async function initialize() {
      if (_config.initialized) return true;
      ...
      try {
        ...
        console.error('[Filter] 初期化エラー:', error);
        return false;
      }
    }
    ...
    async function loadNgWords() {
      try {
        // ハードコードされたNGワードリスト（実際の拡張機能ではファイルからロード）
        _config.ngWords = [
            '◯◯様専用', 'ジャンク', '偽物', '壊れ', '不具合', '初期不良',
            '訳あり', '難あり', '修理', '動作未確認', '返品不可', 'ノークレーム',
            'ノンクレーム', 'ノーリターン', 'すり替え防止', '箱なし', '付属品なし',
            '専用出品', '削除予定', '自宅保管', '確認用', '送料込み', 'まとめ買い',
            'セット販売', '個人購入', '値下げ不可', '交渉不可', '分割',
            'イタズラ', '評価悪い', '評価低い', '取置', '取り置き',
            'ペット', '喫煙', '保管傷', '汚れ', '下着', '使用感', 
            '一部欠品', '完品ではない', '完品でない', '購入意思', '購入意思のない',
            'コメントのみ', 'コメント失礼', 'コメント逃げ', 'サイレント', 'サイレントブロック',
        ...
        // ハードコードされたNG出品者リスト
        _config.ngSellers = [
          'badSeller1', 'problematicUser', 'lowRatingSeller', 'scammerAccount'
        ];
        
        console.log(`[Filter] ${_config.ngWords.length}個のNGワードと${_config.ngSellers.length}名のNG出品者をロードしました`);
        
        // 保存設定から表示/非表示状態を取得
        try {
          const data = await chrome.storage.local.get('hideNgItems');
          if (data.hideNgItems !== undefined) {
            _config.hideNgItems = data.hideNgItems;
          }
        } catch (storageError) {
          console.warn('[Filter] 設定の読み込みに失敗しました:', storageError);
        }
      } catch (error) {
        console.error('[Filter] NGワードロードエラー:', error);
        throw error;
      }
    }
     ...
     * @returns {boolean} NGワードが含まれていればtrue
     */
    function containsNgWord(text) {
      ...
      if (!_config.initialized) {
        console.warn('[Filter] フィルターが初期化されていません');
        return false;
      }
      
      // NGワードをチェック
      for (const ngWord of _config.ngWords) {
     ...
     * @returns {boolean} NG出品者であればtrue
     */
    function isNgSeller(sellerId) {
      ...
      return _config.ngSellers.includes(sellerId);
    }
    
    /**
     * NGワードを含むテキストから、そのNGワードを特定する
     * @param {string} text - チェックするテキスト
     * @returns {string[]} 含まれているNGワードのリスト
     */
     ...
     * @returns {string} NGワードが削除されたテキスト
     */
    function removeNgWords(text) {
      ...
      for (const ngWord of _config.ngWords) {
        cleanedText = cleanedText.replace(new RegExp(ngWord, 'g'), '');
      }
    ...
    /**
     * NGアイテムの表示/非表示を切り替える
     * @returns {boolean} 表示状態 (true: 表示中, false: 非表示中)
     */
    function toggleNgItems() {
      _config.hideNgItems = !_config.hideNgItems;
      
      // 設定を保存
      try {
        chrome.storage.local.set({ hideNgItems: _config.hideNgItems });
      } catch (error) {
        console.warn('[Filter] 設定の保存に失敗しました:', error);
      }
      
      // DOMに適用
      const ngItems = document.querySelectorAll('.ma-tool-ng');
      
      for (const item of ngItems) {
        if (_config.hideNgItems) {
          item.classList.add('ma-tool-hidden');
        } else {
          item.classList.remove('ma-tool-hidden');
        }
      }
      
      console.log(`[Filter] NGアイテムを${_config.hideNgItems ? '非表示' : '表示'}に設定しました`);
      
      return !_config.hideNgItems; // 表示中ならtrueを返す
    }
    
    /**
     * NGアイテムを非表示にすべきかどうかを返す
     * @returns {boolean} 非表示にすべきならtrue
     */
    function shouldHideNgItems() {
      return _config.hideNgItems;
    }
